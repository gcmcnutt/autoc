#!/usr/bin/env python3
"""033 phase-1 M1 per-axis control aggressiveness — single-generation snapshot.

Adapted from 032's per_axis_aggressiveness.py (which adapted 029's). Body
unchanged — column lookup is by header name (outPt/outRl/outTh + Scn +
Pth/Wnd:Step:), present identically in pathgen + tracker data.dat. Only
defaults differ (output path, label).

For the 033 closeout, the **comparison against 029 pastonly3** is the
phase-1 success-criterion variable per spec §2.B Clarifications Q1
("materially better per-axis aggressiveness numbers"). Run twice — once
on the 033 elite gen, once on the 029 pastonly3 elite — and compare
dCtrl per axis.
...
def main():
    ...
    p.add_argument("--pop-size", type=int, default=8000,
                   help="autoc.ini PopulationSize (034 = 8000)")
    p.add_argument("--label", default="034-energy-cleanup")
    p.add_argument("--run-name", default="034-phase1",
                   help="Filename prefix for output PNG.")
    p.add_argument("--out", type=Path, default=None,
                   help="Output PNG path. Defaults to "
                        "specs/034-energy-objective-cleanup/<run-name>_per_axis_aggressiveness.png.")
    args = p.parse_args()

    if args.out is None:
        args.out = Path(f"specs/034-energy-objective-cleanup/{args.run_name}_per_axis_aggressiveness.png")
