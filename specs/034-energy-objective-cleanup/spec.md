...
- **Craft variations do NOT ramp.** Unlike wind/entry variation magnitude (which ramps via `computeVariationScale()`), craft variations are *the craft* — sampled at full magnitude from gen 0. A given scenario flies a fixed airframe draw for the whole run.
...
4. **Given** craft variations are active, **When** the run trains, **Then** craft magnitude does NOT scale with `computeVariationScale()` — it is constant from gen 0 (verified by comparing the variation applied at an early vs late gen for a fixed scenario seed).
...
- **Craft variation accidentally ramps** — must be full magnitude from gen 0; a regression where it picks up `computeVariationScale()` would silently weaken airframe diversity.
...
- **FR-019**: Craft variation magnitude MUST NOT ramp with `computeVariationScale()` — it is applied at full configured magnitude from gen 0 (the craft *is* the craft, not a difficulty curriculum).
...
- **Craft variation**: a deterministic per-scenario airframe-parameter draw (CG, drag, servo responsiveness, total power, trim), jointly sampled with the other variation dimensions, non-ramping, with the seed captured in `ScenarioMetadata`.
...
- **SC-004**: Craft variations are deterministic and reproducible (same craft seed → bit-identical airframe), non-ramping (full magnitude from gen 0), a true no-op at zero magnitude, and the craft seed round-trips through the dmp via `ScenarioMetadata`.
