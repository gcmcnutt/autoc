#!/usr/bin/env python3
"""034 M2 closure-dynamics + sensor-utility analysis.

Carried forward from `specs/032-tracker-nn-enhancements/intercept_analysis.py`;
only the CLI defaults changed (pop-size 5000, 034 label + out path). The data.dat
column contract (spn0/dspn/blC0/brC0/X..Z/tg*/tr*/hull/outTh) is unchanged in 034,
so no field fixup was needed — it reads the live M2 tracker data.dat directly.

032 phase 1 — closure-dynamics + sensor-utility analysis.

Multi-panel figure examining whether the new 032 sensors (spn0 = beacon-pair
NDC span as distance proxy, dspn = one-tick diff as closure-rate proxy)
actually correlate with the NN's throttle policy, and whether they help
avoid overshoots.

Operates on the latest gen present in data.dat — elite-reeval where the
elite NN ran against all 294 scenarios at the end of each generation.
...
Usage:
  python3 intercept_analysis.py [--in data.dat] [--gen N] [--pop-size N]
                                [--label NAME] [--k-scenarios N] [--out PNG]
"""
...
NEEDED_COLS = ["Scn", "Pth/Wnd:Step:", "Idx", "spn0", "dspn",
               "tltS", "tltC", "blC0", "brC0",
               "outPt", "outRl", "outTh",
               "dist", "along", "stpPt", "hull",
               "vxBdy", "vyBdy", "vzBdy",
               "X", "Y", "Z", "tgX", "tgY", "tgZ", "trX", "trY", "trZ"]
...
def load_latest_gen_via_tail(inp, pop_size, scan_bytes=100_000_000):
    """Scan from the END of data.dat (last ~scan_bytes) to find the most
    recent Scn (= latest gen × (pop+1)), then return all per-tick rows
    for that gen keyed by (path, wind).

    One gen ≈ 294 scenarios × ~100 ticks × ~600 B ≈ 18 MB; 100 MB tail
    buffer is comfortable headroom. If gen 0 hasn't completed, fall back
    to a full-file scan."""
    # Header is at line 1
    with open(inp) as f:
        header = f.readline().split()
    idx = {c: header.index(c) for c in NEEDED_COLS if c in header}
    missing = set(NEEDED_COLS) - set(idx.keys())
    if missing:
        print(f"WARNING: missing columns {missing}", file=sys.stderr)

    file_size = inp.stat().st_size
    seek_pos = max(0, file_size - scan_bytes)

    rows = []  # collected (scn, fields) tuples from the tail buffer
    with open(inp) as f:
        if seek_pos > 0:
            f.seek(seek_pos)
        ...
        for line in f:
            fields = line.split()
            try:
                scn = int(fields[idx["Scn"]])
            except (ValueError, IndexError):
                continue
            rows.append((scn, fields))

    if not rows:
        raise RuntimeError("no parseable rows in tail buffer; data.dat empty?")

    max_scn = max(r[0] for r in rows)
    target_gen = max_scn // (pop_size + 1)

    # Aggregate the rows belonging to max_scn
    per_scn = defaultdict(lambda: defaultdict(list))
    for scn, fields in rows:
        if scn != max_scn:
            continue
        pw_step = fields[idx["Pth/Wnd:Step:"]]
        scn_key = pw_step.split(":")[0]
        for c, i in idx.items():
            if c in ("Scn", "Pth/Wnd:Step:"):
                continue
            try:
                per_scn[scn_key][c].append(float(fields[i]))
            except (ValueError, IndexError):
                per_scn[scn_key][c].append(0.0)

    out = {}
    for k, cols in per_scn.items():
        out[k] = {c: np.array(v) for c, v in cols.items()}
        _derive_extras(out[k])
    return target_gen, out


def load_specific_gen_forward(inp, target_gen, pop_size):
    """Stream-scan forward from start. Skip rows until Scn == target_scn;
    collect them; stop as soon as Scn > target_scn (rows are ordered
    Scn-monotonic in practice)."""
    target_scn = target_gen * (pop_size + 1)
    with open(inp) as f:
        header = f.readline().split()
    idx = {c: header.index(c) for c in NEEDED_COLS if c in header}
    missing = set(NEEDED_COLS) - set(idx.keys())
    ...
    with open(inp) as f:
        ...
        for line in f:
            ...
            if scn < target_scn:
                continue
            if scn > target_scn:
                break  # passed the target gen — stop reading
            pw_step = fields[idx["Pth/Wnd:Step:"]]
            scn_key = pw_step.split(":")[0]
            for c, i in idx.items():
                if c in ("Scn", "Pth/Wnd:Step:"):
    ...
    if not per_scn:
        raise RuntimeError(f"gen {target_gen} (Scn={target_scn}) not found "
                           "in data.dat — either out of range or pop_size wrong")
...
def main():
    ...
    ap.add_argument("--gen", type=int, default=-1, help="-1 = autodetect latest")
    ap.add_argument("--pop-size", type=int, default=5000)
    ap.add_argument("--label", default="034-m2-tracker-origm1-r1")
    ap.add_argument("--k-scenarios", type=int, default=6,
                    help="Number of closest-approach scenarios to overlay in panel A")
    ap.add_argument("--out", type=Path, default=None,
                    help="default: specs/034-energy-objective-cleanup/<label>_intercept_analysis.png")
    args = ap.parse_args()
    if args.out is None:  # derive from --label (lexicographic 034-t<N>-<reason> convention)
        args.out = Path(f"specs/034-energy-objective-cleanup/{args.label}_intercept_analysis.png")

    if args.gen < 0:
        print(f"Tail-scanning {args.inp} for latest gen...", file=sys.stderr)
        args.gen, scenarios = load_latest_gen_via_tail(args.inp, args.pop_size)
    else:
        print(f"Forward-scanning {args.inp} for gen {args.gen}...",
              file=sys.stderr)
        scenarios = load_specific_gen_forward(args.inp, args.gen,
                                              args.pop_size)
    print(f"  detected gen = {args.gen}, scenarios = {len(scenarios)}",
          file=sys.stderr)
    ...
    fig.suptitle(f"{args.label} — closure-dynamics + sensor-utility analysis  (gen {args.gen}, elite)",
                 fontsize=12, y=0.995)
