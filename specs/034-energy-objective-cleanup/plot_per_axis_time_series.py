#!/usr/bin/env python3
"""033 phase-1 M1 per-axis aggressiveness time-series — pt/rl/th evolution across gens.

Adapted from 032's plot_per_axis_time_series.py (which adapted 029's).
Body unchanged — column lookup is by header name (outPt/outRl/outTh +
qw..qz + Scn + Pth/Wnd:Step:), all present in pathgen + tracker
data.dat. Only defaults differ (output path, label).

For 033: this is the primary visualization of "is the smoothness penalty
working?" — watch dCtrl per axis trend DOWN over gens. Compare against
029 pastonly3 baseline (smoothness-off) — the 033 line should descend
faster + further on the bang-bang axis (typically roll for the RNN
topology). The phase-1 closeout per spec §2.B Clarifications Q1 is
"materially better per-axis aggressiveness numbers" vs the baseline.
...
def main():
    ...
    ap.add_argument("--label", default="034-energy-cleanup")
    ap.add_argument("--total-gens", type=int, default=800)
    ap.add_argument("--pop-size", type=int, default=8000)
    ap.add_argument("--run-name", default="034-phase1",
                    help="Filename prefix for output PNG.")
    ap.add_argument("--out", type=Path, default=None,
                    help="Output PNG path. Defaults to "
                         "specs/034-energy-objective-cleanup/<run-name>_per_axis_time_series.png.")
    args = ap.parse_args()

    if args.out is None:
        args.out = Path(f"specs/034-energy-objective-cleanup/{args.run_name}_per_axis_time_series.png")
