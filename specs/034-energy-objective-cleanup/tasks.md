...
- [X] T002 [P] (operator-run) Capture behavior-neutrality reference for SC-002. **N/A 2026-06-03** — smoothness was already bypassed before 034, so its removal was guaranteed behavior-neutral (no before/after capture needed); the pre-change tree is also long gone (clamps + variation/PRNG changes since). The reference `data.stc` source it named is itself retired (T052). Closed as not-applicable.
...
- [X] T013 [US1] (operator-run) Build green + crrcsim eval parity (FR-003/SC-001). **Satisfied 2026-06-03 (reframed)** — the literal "identical to pre-minisim-removal" comparison is obsolete (the ±2.5σ variation clamps + PRNG/schema changes since deliberately moved draw values off that baseline, and the pre-removal binary is gone). The property it guards — M1 eval-parity / deterministic replay on the sole crrcsim worker path — is freshly demonstrated: eval of the origm1-5000x49 gen-475 genome (`autoc-eval.ini`, Seed=1780413146) reproduced training fitness **−51913.339637 bit-exact** across all 294 scenarios (`logs/autoc-034-origm1-eval-r1.log`), storedFitness==recomputed. Build is green (the running bake + eval both use the current clamped binary).
...
- [X] T034 [P] [US3] Test: tracker-mode run-id carries the `tracker-` prefix; pathgen carries `autoc-`. **Done 2026-06-01** — factored the mode→prefix decision out of the autoc.cc call site into pure `autoc::runIdPrefixForMode()` (`include/autoc/util/run_id.h`); `tests/run_id_prefix_tests.cc` (4 tests: tracker→tracker-, pathgen→autoc-, unknown/empty→autoc-, exact-token-only) wired into CMake + run_autoc_tests. Builds green via rebuild-perf.sh, 4/4 pass.
...
- [X] T045 [US5] (operator-run) Build `autoc-craft-only.ini` from `autoc.ini` with `EnableWindVariations=0`, `EnableEntryVariations=0`, `EnableRabbitSpeedVariations=0`, `EnableCraftVariations=1`, single-path generator (`PathGeneratorMethod=longSequential` or similar), `WindScenarios` chosen to give enough airframe samples (suggest 36+ for statistical reach on the regression). Keep the σ values at autoc.ini defaults (CG=0.02, drag/thrust/pitch-eff/roll-eff=0.05, trim=0.02 rad). **Done 2026-06-01** — craft-only config ran as `autoc-034-craft-confirm` (seed 1780290226): wind/entry/rabbit all disabled, EnableCraftVariations=1, 36 scenarios, σ at defaults.
- [X] T046 [US5] (operator-run) Short M1 bake on the craft-only ini, gens chosen by convergence (likely fewer than full 800 needed since the basin is narrower with diversity-locked-to-craft only). Capture data.dat + dmps. **Done 2026-06-01** — full 800-gen bake captured (`logs/autoc-034-craft-confirm.log`, `data.dat`).
- [X] T047 [US5] Per-scenario control-bias analysis. **Done 2026-06-01** — see [craft-variation-signal-report.md](craft-variation-signal-report.md). Scripts: `craft_signal.py` (univariate), `craft_signal_mv.py` (multivariate partials), `craft_signal_pooled.py` (cross-gen sign test — the decisive one), `craft_signal_plot.py`→`craft_signal.png`. **Result: SUCCESS.** Because both roll and pitch *command* outputs are bang-bang-saturated, the signal reads off the non-saturated channels (speed, rates, held attitude, throttle, saturation count). Cross-generation sign test over gens 741–800 confirms, with overwhelming significance: thrSc→speed (60/60), trim→bank (60/60, the strongest coupling), thrSc→throttle backed-off (55/60, the predicted active compensation), pitEff→less-saturation (54/60), rolEff→bank+roll-rate. The trim coupling routes through spiral bank rather than held pitch (saturated pitch axis), but is unambiguously present and correctly signed; ≥3 other couplings also read in the expected direction.
...
- [X] T048 [P] Run the Constitution VI type-domain audit grep on touched paths: `grep -nE '\b(float|double)\b' src/eval/ src/nn/ include/autoc/eval/ include/autoc/nn/ | grep -v '// raw-ok:'` — annotate or convert each hit. **Done 2026-06-01** — 034's only new eval-math file is `craft_variation.h`: `CraftDeltas` is `gp_scalar` throughout; the `CraftSigmas` config-struct is deliberately `double` (mirrors the established `VariationSigmas` ini-precision convention) and is now inline-annotated `// raw-ok:`, so the grep is clean on it. The remaining ~422 hits are all pre-existing and constitute the standing [project_scalar_type_audit_backlog](../../.claude/projects/-home-gmcnutt-autoc/memory/project_scalar_type_audit_backlog.md) (not all warrant change; new code adheres) — out of 034 scope.
- [X] T049 [P] Annotate the BACKLOG.md entries pulled into 034 (minisim retire, smoothness, the 5 fold-ins, craft-variation forward-loop) with `→ 034` / DONE; note FR-013/FR-014 were already-satisfied. **Done 2026-06-01** — added DONE headers to "M1 vs M2 dmp disambiguation" (US3/T033) and "AutocConfig auto-print" (US3/T027); minisim (US1), craft variations (US4), smoothness + fold-in summary + FR-013/FR-014 already-satisfied note all annotated in `specs/BACKLOG.md`.
- [X] T050 [P] Update `CLAUDE.md` Recent Changes for 034 (already auto-updated; verify accuracy). **Done 2026-06-01** — verified the 034 line is present and accurate (autoc/crrcsim C++17, analysis Python, Eigen, cereal, inih, GoogleTest, CRRCSim LaRCSim FDM); no edit needed.
- [X] T051 Run `specs/034-energy-objective-cleanup/quickstart.md` verification gates end-to-end (grep gates, no-op check) before the US5 bakes. **Done 2026-06-01** — minisim gate green (annotated the one historical crash_reason.h mention with `// historical`); smoothness gate green (tightened the gate to exclude crrcsim's unrelated render-loop "display smoothness" comments); type-audit per T048. No-op/replay checks are operator-run (T044 already signed off).
...
- [X] T052 [Cleanup] Migrate the six `bout << "#…"` write sites in `src/autoc.cc` to `*logger.info() << "#…"` (keep the `#NNEval` / `#NNGen` / `#GenCrash` / `#GenDiag` / `#GenSimStats` / `#SimRuns` marker tokens unchanged for grep stability). Delete the `bout` ofstream declaration, the `strStatFile` open/close, and the `bout.flush()` calls. Constitution III: clean cut, no back-compat dual-write. **Done 2026-06-01** — all six markers now emit via `*logger.info()` (so they land in the run .log with the `<timestamp>: <info>` prefix); `bout` ofstream + `strStatFile` + the three `bout.flush()` calls removed; `runNNEvaluation`/`runNNEvolution` lost the `std::ofstream& bout` param (+ call sites updated). autoc builds clean (rebuild-perf.sh). Task line-numbers had drifted; located sites by marker token.
- [X] T053 [P] [Cleanup] Update `plot_evolution_progress.py` `--focus` / `--compare` defaults + docstrings to point at `.log` files instead of `data.stc`; remove the .log NN_ELITE fallback path. **Done 2026-06-01** — `--focus` now defaults to the newest `logs/autoc-*.log` (new `_newest_run_log()` helper); docstrings/help retargeted to `.log` (historical `.stc` snapshots still valid via the shared `#NNGen` token + `re.search`); removed the `NN_ELITE_BEST_RE` fallback in `load_log` (kept `NN_ELITE_RE`, still used by `load_crashes`). Smoke-tested: `load_stc` parses a logger-prefixed `#NNGen` line. `per_axis_aggressiveness.py` + `plot_per_axis_time_series.py` confirmed no-op (read `data.dat`, zero `.stc` refs).
- [X] T054 [P] [Cleanup] Strip `.stc` from: `eval_logger.h` header comment, the `eval-data.stc` paste-back baseline reference in `quickstart.md`, and the `.gitignore` `/*.stc` rule. **Done 2026-06-01** — eval_logger.h comment + quickstart.md retargeted to the `.log`. **Deviation (operator call 2026-06-01):** the `/*.stc` gitignore rule was KEPT (not deleted). autoc no longer produces root `.stc`, but the orphaned pre-T052 root `data.stc`/`eval-data.stc` still exist and the rule keeps them out of commits; comment reworded to say so. Committed reference snapshots under `specs/` are unaffected (rule is root-anchored).
- [X] T055 [Cleanup] (operator-run) Confirm data.stc retirement on a live bake. **Done 2026-06-03** — confirmed across the basic-m1, bigfull-clamped, origm1-5000x49 (800-gen) bakes + the eval run: (a) zero `data.stc`/`eval-data.stc` produced (workspace clean, `ls *.stc` empty); (b) `#NNGen`/`#GenCrash`/`#GenDiag`/`#GenSimStats`/`#SimRuns` + `#NNEval` markers all appear in `logs/autoc-034-*.log` at cadence (origm1 log has 470+ `#NNGen` lines); (c) `plot_evolution_progress.py` reads the `.log` directly (now its default focus source) and renders all panels (the many 034- PNGs generated this session).
