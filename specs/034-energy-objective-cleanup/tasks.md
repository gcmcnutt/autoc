...
- [ ] T055 [Cleanup] (operator-run) Run a short bake (≥10 gens) and confirm: (a) no NEW `data.stc` / `eval-data.stc` produced in workspace; (b) the 6 marker lines appear in `logs/autoc-<run>.log` at expected cadence; (c) `plot_evolution_progress.py` against the new .log produces the same panels as the .stc-based version did.
