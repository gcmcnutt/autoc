...
- [X] T037 [US4] Add craft fields to `ScenarioMetadata` (`include/autoc/rpc/scenario_metadata.h`): `craftCGDelta, craftDragDelta, craftTrimDelta, craftThrustScale, craftPitchEffDelta, craftRollEffDelta` (`gp_scalar`) + `uint32_t craftSeed`; append to `serialize()`; NO version revision. Documented insertion point for a future `cameraSeed` (FR-020).
- [X] T038 [US4] Add craft σ knobs (`craftCGSigma, craftDragSigma, craftTrimSigma, craftThrustSigma, craftPitchEffSigma, craftRollEffSigma`, default 0.0) to the `AUTOC_CONFIG_FIELDS` X-macro and a `[Craft]` block in `autoc.ini`/`autoc-tracker.ini`/eval inis. Bump `ContractConfig.ConfigFieldsMacroCount` expectation from 80 → 86.
- [ ] T039 [US4] Create `src/eval/craft_variation.{h,cc}`: fractional-σ Gaussian sampling drawn from `deriveClassSubSeeds(scenarioSeed).craft` (deterministic at full magnitude). `gp_scalar` throughout; populate the `ScenarioMetadata` craft fields + `craftSeed`. Wire into the per-scenario metadata generation (`prefetchAllVariations` in `src/autoc.cc`). Extend `applyVariationScale()` in `include/autoc/eval/scenario_meta_apply.h` to scale the craft fields the same way it scales entry/wind (additive deltas toward 0; `craftThrustScale` interpolated toward 1.0 like `entrySpeedFactor`).
...
- [ ] T041 [US4] Populate the `Global::craft*` carriers at the per-scenario reset hook in `crrcsim/.../inputdev_autoc.cpp:494-520` (alongside entry/wind offsets). Values come from the `meta.craft*` fields *after* worker-side `applyVariationScale()` has scaled them — same as `meta.entry*Offset`.
...
- [ ] T043 [P] [US4] Tests (contracts/craft-variation-contract.md invariants): (a) σ=0 → per-scenario fitness byte-identical to nominal (FR-021); (b) same `scenarioSeed` → identical *drawn* deltas bit-for-bit (FR-018); (c) ramp invariants: drawn delta is identical across gens for a fixed seed, applied delta scales correctly with `computeVariationScale()` at gen 1 vs late gen, eval mode replays saved scale exactly (FR-019); (d) `craftSeed` round-trips dmp `ScenarioMetadata` (FR-020).
