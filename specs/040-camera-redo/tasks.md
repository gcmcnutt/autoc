...
- [x] T088 [P] ✅ **DONE 2026-08-02** — the POV panel now distinguishes the NOMINAL field (its outline) from the EFFECTIVE one. Until now a beacon could sit in a region the airframe permanently blocks and the display looked entirely healthy. **Hatched, not filled** — a fill would bury the beacon dots and range rings the panel exists for; the ticks read as "this area is spoken for" without competing. Two regimes drawn differently because FR-009 distinguishes them: **blocked** (wing/nose, opaque) gets an ✗, **attenuated** (prop disc, reduced but NOT lost) gets a single slash. Computed **once and cached** — obstruction is a property of the configuration, not of any tick — and it calls `testObstruction` rather than reimplementing the geometry, so the overlay cannot drift from what the simulator did. ⚠️ **Stated limitation**: it reads the COMPILED-IN defaults, because the dmp does not carry the airframe/camera config (the standing self-describing-dmp backlog item). Correct for every 040-era dmp; replaying an older run would draw the current airframe over someone else's flight. **The parallax layer that extends this — rings, vertical member, quality rendering, `q` bar — moved to T065d–T065g in Phase 6**, because the Phase 6 checkpoint cannot be read without it
...
- [x] T093 ✅ **PASSED 2026-08-02** — ran `scripts/rebuild-perf.sh` (the *determinism-affecting* gate, which supersedes `rebuild.sh` here since US6 changes perception) on the final tree, log `logs/040-t2-prerun-gate.log`, exit 0. **42/42 suites, 412 tests, 0 fail**; crrcsim links clean. Test count 339 (T001) → 398 (t1 gate) → **412**.
