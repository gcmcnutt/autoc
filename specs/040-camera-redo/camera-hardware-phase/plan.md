...
- **Multi-eval-board de-risk strategy**: prove each chunk of work on its own off-the-shelf eval board *before* the integrated hand-built path. Specifically:
  ...
  - **Recording format** → exercised end-to-end via a Python `specs/040-camera-redo/camera-hardware-phase/beacon-viewer/` utility that consumes UVC frames + writes the canonical `.clip` format. Loader contract is round-tripped before flight gateware lands.
...
| **IV. Unified Build** | Python loader sits in `specs/040-camera-redo/camera-hardware-phase/beacon-loader/` or similar — does not duplicate cereal/inih/GoogleTest declarations. FPGA + MCU builds are explicitly separate (different toolchains; can't unify with CMake). | ✅ PASS |
