...
def compute_anchors(inav_slc, nn, inav_rate_hz=500, span_label=""):
    ...
    # B) QUAT CONSISTENCY
    # msplink.cpp:neuQuaternionToNed stores xiao_quat = conj(inav_mspquat),
    # so for xiao_quat to match INAV's blackbox quat as the same rotation we
    # must conjugate one of them. INAV blackbox quat is body→world (body→earth
    # in INAV terms); xiao_quat is also body→world (that's what the evaluator
    # expects, per aircraft_state.h and evaluator.cc). They should agree up to
    # a sign flip of the whole quat (q and -q represent the same rotation).
    if nn is not None and len(inav_slc['quat']):
        q_inav_at_nn = nearest_rows(inav_slc['t_ms'], inav_slc['quat'], nn['inav_ms'])
        q_xiao = nn['q']
        # Align overall sign (quat and -quat are the same rotation).
        dot = np.sum(q_inav_at_nn * q_xiao, axis=1)
        flip = dot < 0
        q_xiao_aligned = q_xiao.copy()
        q_xiao_aligned[flip] *= -1
        diff = q_inav_at_nn - q_xiao_aligned
        rms = float(np.sqrt(np.mean(diff * diff)))
        a['quat_rms_diff'] = rms
        lines.append(f"B. QUAT xiao↔INAV RMS diff = {rms:.4f}  "
                     + ("✓ consistent" if rms < 0.05 else "✗ DIVERGENT"))
    ...
    if nn is not None and len(nn['tXYZ_now']) > 5:
        ...
        lines.append(f"     mean dtX/dt = {a['mean_dtX_dt']:+.3f} /s   "
                     + ("✓ target coming toward nose" if a['mean_dtX_dt'] > 0
                        else "✗ TARGET MOVING AWAY from nose"))
    ...
    # D) CONTROL CHAIN
    lines.append("D. CONTROL CHAIN (positive corr at short lag = correct polarity):")
    if nn is not None and len(nn['out']):
        ...
        lines.append(f"   NN out  → MSP rc : pitch={a['corr_nn_pitch_rc']:+.2f}  "
                     ...
                     "   (should all be +) [msplink convertXToMSP functions]")
        ...
        lines.append(f"   MSP rc  → rcData : pitch={a['corr_rc_rcData_pitch']:+.2f}  "
                     ...
                     "   (should all be +) [MSP channel mapping]")
...
def anchor_verdict(a):
    ...
    lines.append("")
    lines.append("CONTROL CHAIN (polarity ✓ = positive corr)")
    for label, key in [
        ('NN→rc pitch',      'corr_nn_pitch_rc'),
        ('NN→rc roll',       'corr_nn_roll_rc'),
        ('NN→rc thr',        'corr_nn_thr_rc'),
        ('rc→rcData pitch',  'corr_rc_rcData_pitch'),
        ('rc→rcData roll',   'corr_rc_rcData_roll'),
        ('rc→rcData thr',    'corr_rc_rcData_thr'),
        ('rcData→servo0',    'corr_rcData_servo0'),
        ('rcData→servo1',    'corr_rcData_servo1'),
        ('rcData→motor',     'corr_rcData_motor'),
        ('rcData→gyro p',    'corr_rcData_p'),
        ('rcData→gyro q',    'corr_rcData_q'),
    ]:
        v = a.get(key, float('nan'))
        mark = "✓" if v > 0.2 else ("✗" if v < -0.2 else "?")
        lines.append(f"  {label:<18s} {v:+.2f} {mark}")
    lines.append("")
    if 'mean_cmd' in a:
        p, r, th = a['mean_cmd']
