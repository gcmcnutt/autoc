...
| **M7d.a** | T045 + T046 (partial) | `FitnessComputer` wire-up — cone-surface against trail-rabbit; arena egress as scenario terminator; `arenaEgressCount` + `hullStrikeCount` populated. Crash-hull firing deferred to M7d.b. |
| **M7d.b** | T046 (finish) | Crash-hull probabilistic firing as scenario terminator — `didCrashFire` invocation in `TrackerStepper::stepOnce`; PRNG plumbed from `AutocConfig` through `EvalData` per-scenario; `p_crash` curriculum from `pCrashForGen(gen)`; `hullStrikeCount[i]` increments on fire. |
...
- [ ] T046 [US2] Plug trail-rabbit + crash-hull + arena into the tracker-mode `FitnessComputer` path: cone-surface fitness (existing pathgen `include/autoc/eval/fitness_computer.h` reused) with trail rabbit substituted for path point; crash-hull strike → scenario terminator (Q1); arena egress → scenario terminator (Q3); both record telemetry. (M7d) _(M7d.a / 2026-05-07: PARTIAL — tracker-mode branch in `src/eval/fitness_decomposition.cc::computeScenarioScores` keyed on non-empty `targetTrajectoryList[i]`; rabbit = `trail_rabbit_position`, tangent = unit target velocity (with `prevTangent` fallback for |v|<0.01). Pathgen branch wrapped in `else`, bitwise-preserved (operator confirmed gen9200.dmp eval bitwise-identical 2026-05-07). Arena egress already terminates scenario via `TrackerStepper::stepOnce` (M7a). Per-scenario `arenaEgressCount[i]` = 1 if `crashReason == Eval` (tracker-mode Eval is sourced exclusively from `checkArenaBounds`), else 0; `hullStrikeCount[i]` = 0 placeholder. Smoke run gen 1-2 produced fitness gradient: maxStreak=9, multiplied score=16.99. **Remaining (M7d.b)**: crash-hull `didCrashFire` invocation in `TrackerStepper::stepOnce` + PRNG plumbing through EvalData + p_crash curriculum from `pCrashForGen(gen)` + `hullStrikeCount[i]` increment.)_
