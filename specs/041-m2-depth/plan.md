...
| **II. Build Stability** | ⚠️ **scope grew 2026-08-07** | **Three** build surfaces, not two: `rebuild.sh` (autoc+crrcsim), `pio run -e xiaoblesense_arduinocore_mbed` (xiao), and **INAV** for the `MSP2_AUTOC_STATE` accel extension — the last needing **two variants** (bench = `MAMBAF722_2022A`, flight = `MATEKF722MINI`). Both targets are established and were built together in 021 T041, so this is scope, not risk. |
