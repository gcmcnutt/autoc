...
- [ ] T071 [US4] Run Study A on the new elite and produce the **per-regime intent comparison** against the prior M1's profile from T058 (SC-007a). Report as a **ballpark read**, not an attributable effect size — it is a cross-run profile comparison, not a controlled delta.
...
- [ ] T082 [US5] Produce the **blind-gap distribution** — frequency, duration histogram, exit→re-entry bearing offset. ⚠️ **This must come first**, because it *defines* the go/no-go criterion (FR-024b) and it is a deliverable in its own right regardless of the predictor verdict (it is the input to any future field-of-view decision, and nobody has measured it).
...
- [ ] T094 [US6] Update `include/autoc/nn/topology.h` tracker input count for the innovation channels and add the metadata rows.
...
**Total**: 106 tasks. US1 17 · US2 21 · US3 10 · US4 22 · US5 8 · US6 10 · setup/foundational/polish 18.
