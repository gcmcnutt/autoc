...
- [X] T041f [US2] ✅ **DONE 2026-08-16 — put the grid ON the tape measurement.** `CameraPixelsH = 320 → 312` in the three tracker inis **and both struct defaults** (`CameraConfig::pixels_h`, `AutocConfig::cameraPixelsH`) — the same five-definitions-of-one-value hazard T041d navigated. Grid is now **312 × 200 @ 0.304 °/px ⇒ 94.85° H × 60.8° V**, which sits on 031's tape-measured **95° × 61°** ("MEASURED, FINAL") instead of the f·θ **extrapolation** (97.3°) T041d had kept.
  **Why change what T041d deliberately chose**: T041d kept 320 so the grid was an exact 4× bin of the sensor's full 1280 columns. That is a real property, but it bought it by training against ~2.4° of horizontal field the tape says the lens does not deliver. 312 × 4 = 1248 columns — the bin is now to the **usable** field. Operator 2026-08-16: *"since we haven't yet done an M2, might as well, make sure the constants are around the actual camera."* Free to take precisely because **no M2 has been baked at 97.3°**; after a bake it would have been a mid-comparison change.
  ⚠️ **Fitness-affecting (M2 only)** → A1 bundle, same as T041a/T041d.
  **A physical finding fell out, worth carrying into M2 planning.** The pod-nose obstruction lives at the OUTER EDGE of the horizontal field, and the blocked fraction is steeply field-dependent — measured sweep: onset between 46.2° and 46.8° half-H, then 0.214% at 47.42° (**where we now train**), 0.628% at 48.64° (T041d's field), 2.57% at 54.72°. So the measured lens sees **~1/3 the nose blockage** the extrapolated one did. `AirframeObstructionField.EffectiveFieldDiffersFromNominalByAJustifiedAmount` failed on the change and was **right to**: its `>0.5%` floor and its "~3% is hard-blocked" comment both dated from the retired 120° era, and T041d had already left it passing by a hair (0.628% vs 0.5%). The bound is now stated as the physical claim (non-zero, well under a tenth) with the sweep recorded beside it, so the next field change gets a number rather than a mystery.
  Also added `CameraPixelsH/V`, `CameraDegPerPixel`, `CameraDetectionRangeM` to the train/eval ini-agreement guard: they are M2-only, so M1 pairs skip them, and a tracker train/eval disagreement would now evaluate a policy against a different optic than it trained on. **46/46 suites green.**
