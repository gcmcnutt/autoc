...
- [ ] TA11 Apply TA02's verdict + TA01's ablation: drop what both agree is inert. ⚠️ **TA01 LANDED AND REVERSED THIS.** `ACCEL_Y` is **RETAINED** — it costs **−4.5% on path 5**, more than all three accel axes together, and had the *lowest* contribution score; the proposal to drop it was wrong. The removal candidates are now `IN_ENVELOPE` and `ENVELOPE_SECS` (**+0.3% / −0.2% on path 5** — removing the flag is *better*), pending whether TA09's reshape is a better use of those slots than deletion. ⛔ `DIST_TO_BOUNDARY` and `INWARD_BODY_*` are **RETAINED** by FR-033 regardless of pooled contribution.
