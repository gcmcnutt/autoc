...
- [ ] T203 Renderer legacy INAV blackbox path (WI11): **decided — delete**. Remove `tools/renderer.cc` lines 1720–1735 and downstream dead code. Note for the deleter: this path contains the same full-conjugate bug `(w, -x, -y, -z)` that `xiao/src/msplink.cpp:neuQuaternionToNed` has (see T041). If for some reason the path is kept instead of deleted, the same T041 fix `(w, x, y, -z)` applies — flip qz only (NEU↔NED world-Z reflection), do NOT use the result kinematically. See `docs/COORDINATE_CONVENTIONS.md` "INAV NEU ↔ aerospace NED reflection" section.
