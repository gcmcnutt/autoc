#!/usr/bin/env python3
"""029-no-future-arch variant of join_flight_analysis.

Forked from specs/027-recurrent-nn/join_flight_analysis.py for the
2026-05-03 past-only flight onward.  Schema delta vs the 027 fork:

  - NN sensor input is past-only (5 lookback ticks + now = 6 slots,
    [-5..-1, now] indexed 0..5).  In the xiao log
    "tX=[a,b,c,d,e,f]" the "now" slot is **index 5**, not index 3
    (027 used 3 past + now + 3 future).  All `tX/tY/tZ/d` "now"
    pulls in `nn_arrays()` are updated accordingly.
  - Per-span output adds an aggressiveness summary block
    (per-axis <|out|>, <|Δout|>, near-zero %, saturation %) so that
    cross-flight comparison vs the 04-26 cadence7-redux baseline is a
    table read, not a regrep.

Other notes inherited from the 027 fork:
...
def nn_arrays(nn_rows):
    ...
    return dict(
        ...
        # 029 past-only: 6-slot ring [-5..-1, now]; "now" lives at index 5.
        tXYZ_now=np.stack([
            np.array([r['tX'][5] for r in nn_rows]),
            np.array([r['tY'][5] for r in nn_rows]),
            np.array([r['tZ'][5] for r in nn_rows]),
        ], axis=1),
        dist_now=np.array([r['d'][5] for r in nn_rows]),
...
def compute_anchors(inav_slc, nn, inav_rate_hz=500, span_label=""):
    ...
    # F) BANG-BANG / AGGRESSIVENESS  (per-axis <|out|>, <|Δout|>, near-zero %,
    # saturation % — direct cross-flight comparator vs 04-26 cadence7-redux
    # table in the previous flight report).  Computed on NN tick rate (~20 Hz).
    if nn is not None and len(nn['out']) > 5:
        out = nn['out']  # (N, 3) [pitch, roll, throttle], tanh ∈ [-1, +1]
        dout = np.diff(out, axis=0)
        abs_out = np.abs(out)
        abs_dout = np.abs(dout)
        per_axis_mag = abs_out.mean(axis=0)        # <|out|> per axis
        per_axis_dctrl = abs_dout.mean(axis=0)     # <|Δout|> per axis
        total_mag = float(abs_out.sum(axis=1).mean())   # <|·|sum> per tick
        total_dctrl = float(abs_dout.sum(axis=1).mean())
        # near-zero band [-0.3, +0.3], saturation |·| > 0.95
        near_zero_pct = (abs_out < 0.30).mean(axis=0) * 100.0
        sat_pct = (abs_out > 0.95).mean(axis=0) * 100.0
        a['per_axis_mag'] = per_axis_mag.tolist()
        a['per_axis_dctrl'] = per_axis_dctrl.tolist()
        a['total_mag'] = total_mag
        a['total_dctrl'] = total_dctrl
        a['near_zero_pct'] = near_zero_pct.tolist()
        a['sat_pct'] = sat_pct.tolist()
        lines.append("F. BANG-BANG / AGGRESSIVENESS  (per-axis, NN tick rate):")
        lines.append(f"   <|out|>     pt={per_axis_mag[0]:.2f}  rl={per_axis_mag[1]:.2f}  "
                     f"th={per_axis_mag[2]:.2f}    sum/tick={total_mag:.2f}")
        lines.append(f"   <|Δout|>    pt={per_axis_dctrl[0]:.2f}  rl={per_axis_dctrl[1]:.2f}  "
                     f"th={per_axis_dctrl[2]:.2f}    sum/tick={total_dctrl:.2f}")
        lines.append(f"   near-zero%  pt={near_zero_pct[0]:4.0f}  rl={near_zero_pct[1]:4.0f}  "
                     f"th={near_zero_pct[2]:4.0f}     (|·|<0.30)")
        lines.append(f"   sat %       pt={sat_pct[0]:4.0f}  rl={sat_pct[1]:4.0f}  "
                     f"th={sat_pct[2]:4.0f}     (|·|>0.95)")
