...
- [X] T013 [OP] P0-G: **DONE 2026-07-05** — M1 (t3, `autoc-m1/...9223370253844606963...`) ≈ parity with t10; M2 baseline (t4, `autoc-m2/...9223370253664873750...`, stopped gen 307 plateaued) sanity-check PASSED + reproduces the perception-capped ceiling. Payoff: no isolable anti-target-overrun from FR-P0H (hull=0 is the penalty, not the sensors); recurrence engaged+inflating but depth-flat ⇒ capacity not the limiter, anticipation is ⇒ motivates US3. Baselines are the ablation `--compare` references (t3 M1 clean; t4 M2 partial-but-at-ceiling). See outcome.md. — re-bake a fresh **enriched** M1 source (post-P0-D format incl. T009b situational-awareness inputs) `scripts/train.sh autoc.ini logs/autoc-038-t1-m1-rebake.log`, pin the winner in `autoc-m1`, record the S3 prefix in `outcome.md`; then re-bake the M2 baseline `scripts/train.sh autoc-tracker.ini logs/autoc-038-t2-m2-baseline.log`. **Measure the enriched baseline vs the old (037 t11/t14) M1+M2 baseline** (FR-P0H): did overrun drop / boundary behavior improve? Record in `outcome.md` — this is the situational-awareness payoff, separate from the architecture ablations. **Unconditional** (Clarifications 2026-07-01): the inputs stay regardless — a null result is recorded, NOT reverted (no escape gate; reverting would force a second break).
...
- [X] T023 [US3] **DONE 2026-07-05** (impl, clean rebuild-perf green, commit `41a3d3a`) — output head 3→7 (tracker only; pathgen untouched, no xiao — predictor is M2-direct), `nnOutputs_[7]`, aux [3..6] recorded via the **SPLIT** AircraftState serialize (pathgen/tracker each carry only their own NN block — infra that ends the forced M1-source re-bake on every M2 format change), dmp_dump aux cols spP1/spP2/spP3/spdR. **Target = ego-invariant `span`/closure @ {50,100,150} ms (ms-based, cadence-invariant), NOT raw NDC** (Clarifications 2026-07-05). — grow output head 3→7 in `include/autoc/nn/topology.h` (`*_OUTPUT_COUNT`, `*_WEIGHT_COUNT`); resize `nnOutputs_[]` + deserialize output-count check in `include/autoc/eval/aircraft_state.h:580`; output buffers in `include/autoc/nn/evaluator.h` + `src/nn/evaluator.cc` (outputs[0..2]=control, [3..6]=predicted optical; aux not actuated). **The dmp is the per-tick trace** (data.dat retired in 035 FR-P05); the resized `nnOutputs_` records the aux outputs honestly via `AircraftState` cereal, but the CSV dump tool has a hardcoded 3-output header — extend `out_pt,out_rl,out_th` in `tools/dmp_dump.cc:68` to surface the 4 aux columns so they're inspectable (honest-recording audit, [feedback_honest_dmp_recording])
- [X] T024 [US3] **DONE 2026-07-05** (impl, commit `41a3d3a`) — `prediction_score` on `ScenarioScore`; `computeSpanPredictionError` (fitness_decomposition) = mean |predicted−realized span| over CEP-visible (t, t+horizon) pairs + closure-rate; SEPARATE lexicase axis, gated via `include_prediction_axis` param (not scalar-composited). — add `prediction_score` (`gp_fitness`, negated) to `ScenarioScore` in `include/autoc/eval/fitness_decomposition.h:71`; accumulate per-tick aux-vs-realized-next-tick optical error in `src/eval/fitness_decomposition.cc`; add it as a SEPARATE per-scenario axis in `src/eval/selection.cc` (NOT scalar-composited — per [contracts/lexicase-axis-and-reward.md](contracts/lexicase-axis-and-reward.md))
- [X] T025 [US3] **DONE 2026-07-05** (impl, commit `41a3d3a`) — `EnablePredictorHead` knob (field count 96→97); `applyCrashPenalty` touches only `.score` (verified); fixtures regen; `tests/predictor_head_tests.cc` (layout + cadence-invariant horizons + prediction-axis selection). xiao codegen N/A (tracker not on xiao). — add `EnablePredictorHead` knob to `AUTOC_CONFIG_FIELDS` in `include/autoc/util/config.h` (no in-class default per VII); verify `applyCrashPenalty` (`src/autoc.cc:181`) leaves `prediction_score` untouched; regen `tests/nn_*`, xiao codegen; add a prediction-axis selection unit test
- [X] T026 [US3][OP] **DONE** — pre-run build gate + bitwise gate (clean build; determinism re-confirmed 2026-07-08 by the knob-off eval reproducing t6 gen344 `-13382.484965` bit-exact, `NN_EVAL_SAME`)
- [X] T027 [US3][OP] **DONE (bake = t6, stopped gen 344/800 2026-07-08)** — `logs/autoc-038-t6-m2-predictor.log` (`autoc-m2/...9223370253417148111...`, seed 1783437625). **🚦 US3 gate did NOT clear**: the predictor forms weakly at best (in-streak accuracy briefly emerged ~gen 210 then *reversed*, span never calibrated, closure-rate ceiling-bound at ±1 tanh) and **did not lift the perception ceiling** — t6 rode exactly on the t4 baseline plateau (perception ~0.70, range ~17 m). Root read = the aux head is **non-actuated** + weak tie-break selection → no perception→control coupling. See outcome.md "US3 predictor (t6) — result @ gen 344". **Verdict PROVISIONAL**: a foundational confound surfaced — the M2 chase flew an *independent* wind/craft/entry from the source ("good tracking but in beyond-physics envs") — now under test by the **t7 shared-airspace A/B** (`logs/autoc-038-t7-m2-shared-env.log`, `TrackerChaseUseSourceScenarioSeed=1`; see BACKLOG "038 deferrals"). Finalize the US3 escape after t7.
...
- **Phases 3 & 5 (initial-wave ablations)**: independent of each other — **US1 and US3 run in parallel** off the T013 baseline (separate bakes; the operator paces concurrency). Each self-gates with escape. **Phase 4 (US2) is deferred** — not launched in the initial wave (T021/T022 held for unpark). ⚠️ **Sequencing under reconsideration (2026-07-03)** — the learnable-feedback lean argues for **US3-lead** (learnable anticipation, the real anti-overrun bet), demoting US1 (deeper history = doubling the hand-built-memory fudge) to a gated M1-only control or dropping it for un-parking US2. See `specs/BACKLOG.md` → "038 deferrals" → learnable-feedback direction; decide once the M1 (t3) + paired M2 baselines have signal.
