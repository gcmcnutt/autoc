#!/usr/bin/env python3
"""033 #GenDiag multi-panel chart — operator-instrumentation per-gen diagnostics.

NOTE: #GenDiag lines are emitted by tracker-mode (M2) only. In 033 phase 1
M1 (pathgen) runs the data.stc only has #NNGen / #GenCrash / #GenSimStats
— this script will report "no #GenDiag lines found" gracefully. Keep
this script around for the 033 phase-2 M2 inheritance bake (deferred
post-Sunday-flight).
...
def main():
    ...
    ap.add_argument("--out", type=Path, default=None,
                    help="Output PNG path. Defaults to "
                         "specs/033-m1-smooth-plus-variations/<run-name>_gen_diag.png.")
    ap.add_argument("--variation-ramp", type=int, default=40,
                    help="vertical-line marker every N gens (default 40)")
    args = ap.parse_args()

    if args.out is None:
        args.out = Path(f"specs/033-m1-smooth-plus-variations/{args.run_name}_gen_diag.png")
