...
def main():
    ...
    p.add_argument("--run-name", default="033-phase2",
                   help="Filename prefix for output PNG. Track multiple bakes "
                        "(e.g., 033-phase1 vs 033-phase2) without overwriting.")
    p.add_argument("--out", type=Path, default=None,
                   help="Output PNG path. Defaults to "
                        "specs/033-m1-smooth-plus-variations/<run-name>_per_axis_aggressiveness.png.")
    args = p.parse_args()

    if args.out is None:
        args.out = Path(f"specs/033-m1-smooth-plus-variations/{args.run_name}_per_axis_aggressiveness.png")
