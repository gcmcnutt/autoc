...
def main():
    ...
    p.add_argument("--focus", type=parse_pair, default=("033-m1", Path("data.stc")),
                   help="focus run NAME:STC_PATH (default 033-m1:data.stc)")
    p.add_argument("--compare", type=parse_pair, action="append", default=[],
                   help="comparison run NAME:STC_PATH (may repeat). Pass "
                        "029-pastonly3:specs/029-no-future-arch/pastonly3.stc "
                        "for the M1 smoothness-off baseline comparison "
                        "(the phase-1 closeout gate per spec §2.B).")
    p.add_argument("--run-name", default="033-phase2",
                   help="Filename prefix for the output PNG. Track multiple bakes "
                        "(e.g., 033-phase1 = floor=0.5 stalled; 033-phase2 = "
                        "floor=1.0 plumbing-sanity) without overwriting prior PNGs.")
    p.add_argument("--out", type=Path, default=None,
                   help="Output PNG path. Defaults to "
                        "specs/033-m1-smooth-plus-variations/<run-name>_evolution_progress.png.")
    ...
    if args.out is None:
        args.out = Path(f"specs/033-m1-smooth-plus-variations/{args.run_name}_evolution_progress.png")
    ...
    title = args.title or f"{focus_name} — 033 M1 phase-1 evolution progress"
