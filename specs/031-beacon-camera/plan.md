...
**Status (2026-06-24):** **M0 ✓.** **M1 largely met on the in-FPGA correlator-sim harness** (S1–S3c, [`firmware/beacon-decoder-stepfpga/correlator-sim-plan.md`](../../firmware/beacon-decoder-stepfpga/correlator-sim-plan.md)): emitter, virtual-MCP3201 + soft SPI, soft correlator + DC-removal/AGC + min-lock/limited-hold FSM + dual-code discrimination — all HW-verified. **UART `BCN` telemetry now DONE** (2026-06-24): FPGA TX on pad **A2** → STEPLink → COM3, host parser ingests live frames (`uart_bcn.v`; F3/S4). **Command RX now DONE** (pad A3, host→FPGA knob byte; `host/cmd_read.sh`) — bidirectional link complete, A4d sweeps scriptable. **Pending for M1:** the **per-beacon DPLL** and lock vs a **live** emitter. **M3** (two *simultaneous* emitters / true CDMA) + the **A4d** code-length/latency/dropout study are the next frontier; N=15 looks under-spec.
...
- **FPGA IP** — only the MachXO2 PLL is library IP (Diamond); the rest is custom RTL (fpga §3). NB the hard
  PLL is a **MHz clock generator** — it has no role in tracking the 200 Hz beacon rate; the per-beacon DPLL is
  an **all-digital NCO** in fabric (small, kHz-class).
- **Beacon DPLL frequency-flywheel / coast** — re-acquire after occlusion is phase-only if the DPLL HOLDS the
