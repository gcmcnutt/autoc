...
**Status**: ⚠️ **RE-SCOPED 2026-06-20** — 031 is now the **1-bit single-IR-sensor acquisition-research phase**; the active plan is [`acquisition-research-plan.md`](acquisition-research-plan.md) and tasks are **Phase A** in [`tasks.md`](tasks.md). This spec (the full camera pipeline: camera + lens + filter + recorder + paired-craft flight) is the **camera-redo feature `040`** (emitters unchanged; single-sensor analog front end replaced with a camera + bigger FPGA; **040 home:** [`../040-camera-redo/README.md`](../040-camera-redo/README.md)). It remains the camera-phase reference; do not build the camera chain from it under 031. _(Original: DRAFT — spec + clarify only; bench experiments iterate it once hardware lands.)_

> **BASELINE CORRECTION 2026-06-22** — the camera-era operating point in this doc is now **20 Hz control loop (037), 480 fps camera, 200 Hz Gold-code chip rate, 75 ms (15-chip) code period, 15-bit codes**. This supersedes the original **10 Hz / 240 fps / 100 Hz / 150 ms** figures throughout. The forward-looking requirement text (FR-1.3, FR-2.x, NFR-4, Decisions-Locked) has been updated to the new baseline; **dated entries in the Clarifications log and Revision History below preserve the original numbers as historical record** and carry inline supersession markers. Canonical derivation: 200 Hz × 2.4 frames/chip = 480 fps; 15 chips @ 200 Hz = 75 ms = 1.5 ticks @ 20 Hz. See [`acquisition-research-plan.md` §7](acquisition-research-plan.md) (the active 1-bit phase) for the rationale; 240 fps / 100 Hz is the legacy low-rate point, no longer the acquisition baseline.
**Input**:
...
- Q: With 15-bit Gold code, what's the sensible chip rate considering the 10 Hz FC sample rate? → A: **100 Hz chip rate (firm)** — 150 ms code period = ~1.5 FC samples cold acquisition. 240:100 = 12:5 non-integer ratio sidesteps the prior 48 Hz / 240 fps 5:1 integer-aliasing edge case. 2.4 frames-per-chip oversampling preserves matched-filter margin. Ongoing position-fix updates at 240 fps regardless (only CEP confidence is gated by code-period rate). **[Superseded 2026-06-22 → 200 Hz chip / 480 fps camera / 75 ms code period / 20 Hz control loop; the 12:5 ratio and 2.4 fpc oversampling are preserved at the new rates. See FR-1.3 + acquisition-research-plan §7.]**
...
- **A single mounted camera** with a wide-angle (~120° H FOV) M12 lens, **850 nm bandpass filter** (≤30 nm FWHM) integrated in the lens housing, and a global-shutter mono sensor at **320×240 @ 480 fps** (200 Hz chip rate × 2.4 frames/chip; see FR-1.3).
- **A raw-frame recording capability with two modes**: (a) **bench mode** — USB-tethered host PC for early bring-up and bench scenario sweeps; (b) **flight mode** — onboard SD flash card recording at the **36.9 MB/s** sustained data rate (320 × 240 × 1 B × 480 fps), V60 minimum / V90 preferred class, auto-start on power-up so a flight-only operator does not touch the recorder mid-mission. Both modes produce the same file format + JSON sidecar.
...
Acceptance: **320×240 raw mono frames at 480 fps** captured to disk for a configurable duration, with per-frame microsecond timestamps. Sun/ambient/range manually logged. Existence of two distinguishable IR blobs in the recorded frames at design ranges confirms the optical chain works end-to-end.
...
internal JST-PH 2.0 mm socket → 1S rail (V_BAT)
   ...
   │      - Soft-start = 20 µs (datasheet SU) — negligible vs 5 ms chip period (200 Hz)
   ...
   │      │ ─── Gold-code LUT bit at 200 Hz chip rate, GPIO push-pull active-HIGH:
...
| Diagnostic LED (**mandatory** per US6 pre-flight check) | 0603 green visible-light LED | 0603 | 1.6 × 0.8 mm | 0.5 mm | Blinks at the Gold-code chip rate (200 Hz) for bench- and field-side verification that the pod is alive + emitting; uses ~3 mA. Visible through a small light-pipe slot on the pod enclosure's inboard face (operator-visible at pre-flight walk-around) |
...
The code generation contract (chip rate **200 Hz** per the 480 fps / 20 Hz camera-era baseline — updated 2026-06-22, see [acquisition-research-plan.md §7](acquisition-research-plan.md); supersedes the 2026-05-14 figure of 100 Hz / 240 fps / 10 Hz FC. Per-LED current 300 mA per FR-1.4):

- **Chip rate**: **200 Hz (firm).** 15-chip code period = 15 / 200 = **75 ms** = **1.5 control ticks at the 20 Hz control loop** (037) for cold-acquisition latency. Frames-per-chip oversampling at 480 fps camera = **2.4 fpc** (480 / 200 = 12:5 non-integer ratio — see "Aliasing avoidance" below).
- **Why 200 Hz** (camera-era baseline; supersedes the prior 100 Hz / 240 fps point, which itself replaced a 48 Hz default):
  - **Cold-acquisition latency**: 75 ms = ~1.5 ticks at 20 Hz. The NN controller transitions through `no_lock → tentative_lock → confirmed_lock` within ~2 control ticks, which feels instantaneous to the operator. Early/partial-code acquisition (~70% of the code) trims this toward ~1 tick (acquisition-research-plan §7).
  - **Oversampling**: 2.4 frames-per-chip at 480 fps is comfortable Nyquist margin and tolerates motion blur + frame drops. (The prior "≥5 fpc" rule was conservatively over-budgeted given the camera_considerations.md SNR analysis showing 70+ dB margin at 100 m daylight.)
  - **Aliasing avoidance**: 480:200 = 12:5 non-integer ratio means camera-frame edges never sit at fixed offsets relative to chip-transition edges. (The prior 5:1 integer ratio at 48 Hz had a theoretical persistent phase-alignment edge case.)
...
- **Hard floor (Nyquist)**: chip rate ≤ camera_fps / 2 = 240 Hz at 480 fps. The 200 Hz default sits comfortably below this. Bench-iteration range: 100 Hz to 240 Hz, firmware-tunable.
- **Code length**: **15 chips** → one full code period = 15 × chip_period (= 75 ms at the 200 Hz default). Family of 17 N=15 Gold codes available, far more than the 2 the pair needs; cross-correlation bounded by 9/15 ≈ −4.4 dB peak vs −15 dB nominal (verify per pair selection offline). Acquisition error tolerance: 1 bit worst case at N=15 vs 0 bits at N=7 (camera_considerations.md table). This is the headline reason for the bump to 15.
...
- **31-bit upgrade path** (future, NOT this phase): if the follow-on noise-cal or multi-beacon spec needs more codes (>4) or stronger error tolerance (5 bits vs the current 1 bit), bump to **31-bit Gold codes at the same 200 Hz chip rate / 480 fps camera**. 31 chips @ 200 Hz = ~155 ms code period (~3 control ticks at 20 Hz; partial-code acquisition keeps re-lock inside budget), gaining a 31-code family + 5-bit error margin. SD bandwidth is **unchanged** — already at the 480 fps baseline — so the upgrade is pure code-length: the pod firmware changes the LUT length only, hardware unchanged. **Documented here as a known clean upgrade path** so future specs can reference it without re-deriving.
...
- (a) **Scope-level**: capture one pod's LED-string current (via sense-resistor) waveform on a fast scope, confirm the chip rate (**200 Hz, ~5 ms chip period**) + 15-chip code period (75 ms) + correct Gold-code pattern;
...
6. **Chip-rate placement**: the **200 Hz Gold-code chip rate** sits just below typical FC gyro low-pass filter cutoffs (≥250 Hz) — its fundamental is now near the gyro passband edge (vs comfortably clear at the legacy 100 Hz), but the LED-string modulation radiates weakly (constant-current DC switched on/off at 200 Hz, not a fast-edge switch node), so coupling into the gyro path stays minimal; FR-3.3 bench-EMC confirms. The boost-converter 1.6 MHz fundamental + harmonics remain the dominant radiated-EMI concern; they are what mitigations 1–4 target.
...
| Frame rate | **480 fps at 320×240 (firm baseline)** — required for 2.4 frames/chip at the 200 Hz chip rate; 240 fps retained as a legacy low-rate option | FR-2.3 |
...
|---|---|---|---|---|---|
| **480 fps mode (baseline)** | 320 × 240 mono | **480 fps** | 8-bit (baseline) or 10-bit-packed | **36.9 / 46.1 MB/s** | **V60 / V90** |
| **240 fps mode (legacy low-rate)** | 320 × 240 mono | 240 fps | 8-bit (baseline) or 10-bit-packed | 18.4 / 23.0 MB/s | V30 / V60 |

**Why both modes**:

- **480 fps + 15-bit Gold + 200 Hz chip rate** is the Gold-code-acquisition baseline (FR-1.3 link-budget design point), giving the 2.4 frames/chip oversampling the matched filter needs. 480 fps also buys (a) the 31-bit-Gold-code upgrade path (FR-1.3) at no extra bandwidth, (b) close-range motion-blur characterization during fast body rotations, (c) AGC-dynamics studies through sun→shadow transitions where ~2 ms time resolution matters. Native on the OG0VA primary sensor; bench-verify achievable rate on OV9281 backup.
- **240 fps** is the legacy low-rate point: at 200 Hz it gives only 1.2 frames/chip — below the 2 fpc Nyquist-margin floor — so it is usable only at a reduced ≤100 Hz chip rate (the original baseline) or for non-acquisition imagery. Kept for backward comparison and bandwidth-constrained bench work.
...
**SD card** (firm): for the **480 fps baseline** operating mode, **V60 minimum (V90 for 10-bit-packed at 46 MB/s)** UHS-I/UHS-II microSD, 64-128 GB. Recommended specific parts:
- **SanDisk Extreme Pro microSD V60 64-128 GB** (rated ≥90 MB/s sustained write — comfortable margin over the 36.9 MB/s 8-bit / 46.1 MB/s 10-bit sustained rate; V90/UHS-II for the 10-bit path)
- **Kingston Canvas React Plus V90** alternate for the 10-bit path
- For the legacy 240 fps low-rate mode only: V30 (e.g., SanDisk Extreme Pro V30) suffices (18-23 MB/s).

The flight-recorder firmware SHALL:

- Use **4-bit SDIO @ ≥25 MHz** on the SD interface (≥12.5 MB/s theoretical; SDIO drivers in Lattice's Propel reference designs hit ~40 MB/s sustained in practice). **1-bit SPI mode is insufficient** (max ~10 MB/s — far below the 480 fps requirement of 36.9-46.1 MB/s).
...
| **SD card** | **SanDisk Extreme Pro microSD V60 64-128 GB** (480 fps baseline; V90/UHS-II for 10-bit) — V30 only for the legacy 240 fps low-rate mode | Amazon / B&H / Adorama | <1 g | Per FR-2.5 data-rate table |
...
Bench-mode is **completely decoupled from the Lattice flight-mode gateware** — it works as soon as the USB camera is in hand, before any FPGA bring-up. Used for FR-3.1 / FR-3.2 / FR-5.1 indoor + outdoor bench sessions where a host PC is co-located. **Frame-rate caveat**: UVC over USB 2.0 high-speed gives sustainable ~30-60 fps live display at 320×240; recording to disk can hit higher rates limited by the host's USB stack. The 480 fps baseline (and the legacy 240 fps) high-rate modes are flight-mode (FR-4.1b) features — bench mode trades temporal resolution for instant-on usability.
...
- ±5% drift over the 15-chip 200 Hz code period: chip period of 5 ms drifts by ±0.25 ms; cumulative phase error across 15 chips = ±3.75 ms = ±1.8 camera frame periods (at 480 fps — the frame-count tolerance is unchanged from the legacy 100 Hz / 240 fps point because chip period and frame period both halved).
...
| **Chip rate: 200 Hz** _(updated 2026-06-22 from 100 Hz; camera-era 480 fps / 20 Hz baseline — see FR-1.3 + acquisition-research-plan §7)_ | clarification 2026-05-14, revised 2026-06-22 | 15-chip code period = 75 ms = ~1.5 ticks at the 20 Hz control loop for cold acquisition. 480:200 = 12:5 non-integer ratio avoids the prior 5:1 aliasing edge case. 2.4 fpc oversampling at 480 fps preserves matched-filter margin |
...
| **31-bit Gold codes as future upgrade path** _(updated 2026-06-22: 480 fps is now the baseline, not part of the upgrade)_ | clarification 2026-05-14, revised 2026-06-22 | Clean path forward for future multi-beacon (>4) or stronger error tolerance (5-bit vs 1-bit at N=15). Stay at **15-bit / 480 fps / 200 Hz** for this transitory M2 effort; the 31-bit upgrade is pure code-length at the same fps/chip-rate (no extra SD bandwidth). Documented in FR-1.3 |
...
| **Camera operating modes: 480 fps (baseline, 36.9-46 MB/s) + 240 fps (legacy low-rate, 18.4-23 MB/s); both at 320×240 mono; 8-bit or 10-bit-packed selectable** _(baseline flipped to 480 fps 2026-06-22)_ | clarification 2026-05-14, revised 2026-06-22 | 480 fps is the FR-1.3 Gold-code-acquisition baseline (2.4 fpc @ 200 Hz) and supports the 31-bit upgrade + close-range motion-blur + AGC-dynamics studies; 240 fps gives only 1.2 fpc @ 200 Hz so it is the legacy/bandwidth-constrained mode. Both natively on OG0VA |
...
| **SD card: SanDisk Extreme Pro microSD V60 64-128GB (480 fps baseline; V90/UHS-II for 10-bit) / V30 only for legacy 240 fps** _(baseline bumped to V60 2026-06-22)_ | clarification 2026-05-14, revised 2026-06-22 | Matches the FR-2.5 data-rate table; V60 covers the 36.9 MB/s 8-bit baseline, V90 the 46 MB/s 10-bit path |
...
| **Camera fps: 480 (firm baseline)** _(updated 2026-06-22 from 240)_ | user direction 2026-05-12, revised 2026-06-22 | Sets the FR-1.3 chip-rate derivation (200 Hz × 2.4 fpc); 240 fps is the legacy low-rate point. Matches camera_considerations link-budget analysis |
...
| **Recording substrate: dual mode — bench (USB-tethered to Linux PC) + flight (onboard SD, V60 min / V90 for 10-bit, 36.9 MB/s sustained at the 480 fps baseline)** _(rate/class updated 2026-06-22)_ | user direction 2026-05-12, revised 2026-06-22 | US6 (Beacon Test Flight 1) requires onboard recording; bench mode unchanged from prior draft |
...
| **Clip-file durability: pre-allocated file + chunked direct-sector writes (~1 s blocks); each chunk independently parseable; ~1 s trailing-frame loss accepted on power-loss/crash. Raw-LBA fallback permitted if FS bookkeeping bottlenecks the 36.9 MB/s 480 fps baseline rate** | clarification 2026-05-12, rate revised 2026-06-22 | Standard dashcam-/black-box-recorder pattern; loses last second on hull crash (acceptable per operator); avoids per-frame fsync overhead |
...
- Q7 (chip rate): **200 Hz firm; 15-bit Gold; 480 fps camera** _(updated 2026-06-22 from the original 100 Hz / 240 fps — camera-era 20 Hz / 480 fps baseline; see FR-1.3 + acquisition-research-plan §7)_.
...
- **Revised**: 2026-06-22 — **rate-baseline correction** to align with 037 (20 Hz control loop GO) and the 038 M2 CEP model: camera-era operating point updated from **10 Hz / 240 fps / 100 Hz chip / 150 ms** to **20 Hz / 480 fps / 200 Hz chip / 75 ms** (15-bit codes unchanged; 200 Hz × 2.4 fpc = 480 fps; 15 chips @ 200 Hz = 75 ms = 1.5 ticks @ 20 Hz). Operative requirement text (FR-1.3, FR-2.x, NFR-4, Decisions-Locked, Q7) rewritten; dated Clarifications-log entries annotated with supersession markers (history preserved). 240 fps / 100 Hz is now the legacy low-rate point. Rationale lives in [`acquisition-research-plan.md` §7](acquisition-research-plan.md) (the active 1-bit acquisition phase). Companion fix: 038 spec.md CEP smear-window arithmetic (150 ms → 75 ms @ 480 fps).
