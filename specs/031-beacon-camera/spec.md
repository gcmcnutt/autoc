...
internal JST-PH 2.0 mm socket → 1S rail (V_BAT)
   ...
   │   │   EN HIGH (boost warm) whenever V_BAT > 3.3 V; LOW (shutdown) below
   │   │
   ...
   │      - EN  ←─ supervisor (held HIGH in normal op = boost warm)│
   │      - DIM ←─ MCU GPIO (digital on/off, code LUT bit at 100 Hz)│
   ...
   └── ATtiny412 MCU runs directly off V_BAT (1.8-5.5 V tolerant — no LDO)
          ...
          │ ─── Gold-code LUT bit out at 100 Hz chip rate ──→ LED driver DIM pin →
          ▼
      (LED driver IC handles the CC switching; MCU only drives a single digital line
       — no PWM peripheral, no analog filter, no dim-duty-cycle. Boost stays warm via
       supervisor-held EN; DIM toggles the LED current cleanly between 0 and 300 mA
       at each chip edge with no soft-start re-trigger.)
      (battery removal = V_BAT collapses = MCU + supervisor lose power = EN drops,
       DIM goes high-Z = LEDs off)
```
...
| Boost LED driver IC | **TI LM3410-Y** (primary, 1.6 MHz fsw) / **TI TPS61169** (alternate, smaller package) | SOT-23-5 | 3 × 3 mm | 1.1 mm | 2.7–5.5 V input, up to 38 V output, 1.6 A internal switch; current-mode PWM (internal boost regulator switching — NOT a dim PWM); FB pin regulates LED current via external R_sense. **EN pin is held HIGH by the voltage supervisor whenever V_BAT > 3.3 V — boost regulator stays warm in normal operation; supervisor drops EN at UVLO.** **DIM pin is driven as a digital on/off gate by the MCU GPIO at the 100 Hz chip rate (no PWM dim control — strict on/off only).** **-Y (1.6 MHz)** variant enables the smaller 22 µH inductor. Built-in 2.5 V UVLO is **insufficient** for LiPo protection — the external supervisor handles the 3.3 V cutoff (FR-1.7 #4) |
...
| MCU | **Microchip ATtiny412** | SOIC-8 | 5 × 4 mm | 1.5 mm | 8 pins: V+, GND, **code-out GPIO (single digital line to LED driver DIM — strict on/off LUT bit at 100 Hz chip rate; no PWM peripheral required)**, 2× code-select inputs, UPDI, reset, 1 spare. MCU does NOT drive the boost driver's EN pin — that is the supervisor's exclusive domain (FR-1.7 #4). **Runs directly off V_BAT** (1.8-5.5 V operating range covers the 3.0-4.2 V LiPo swing); no LDO needed. Internal 20 MHz RC oscillator — see NFR-4 clock-drift requirements |
...
Mechanism: the TI LM3410-Y boost driver (FR-1.2.1) switches at **1.6 MHz**; the switch node carries ~1 A swings at sub-µs edges. Without mitigation, the switch-node loop radiates a broadband spectrum extending into the VHF range. The LED-string output current is comparatively benign — constant-current DC at 300 mA, modulated only at the 100 Hz Gold-code chip rate (~10 ms chip period) — easy to filter.
...
4. **Battery low-voltage cutoff (LiPo protection — safety-critical)**: the pod SHALL stop driving the LEDs when the 1S cell drops to **3.3 V** (firm UVLO threshold, see Decisions Locked). This is **mandatory hardware-level UVLO** — must trip independently of the MCU (so a crashed/hung MCU does not strand the boost driver still draining the cell into damage range). Implementation contract:
   ...
   - **Voltage supervisor IC** (e.g., **Microchip MCP1316T-29LE/OT**, **TI TPS3839K33**, **Diodes APX803-31SAG**) monitors the 1S rail and gates the boost LED driver's **EN (SHDN) pin** — **only the supervisor touches EN**; the MCU does not have a wire to EN. Open-drain output preferred so the supervisor pulls EN low at the trip point. In normal operation (V_BAT > 3.3 V), EN is held HIGH and the boost regulator stays warm; the MCU's chip-rate on/off modulation on DIM does not affect EN and does not retrigger the boost soft-start.
   ...
   - **NOT firmware-controlled**: the MCU does NOT participate in the cutoff path. Even if the MCU crashes, hangs, or holds DIM stuck-HIGH, the boost driver SHALL still shut off at the supervisor threshold (supervisor drops EN, which kills the boost regardless of DIM state). The MCU MAY monitor a separate ADC reading of the 1S rail for diagnostic blink-codes (FR-1.2.1 diagnostic LED) but the supervisor is the load-bearing safety element.
...
| **LED drive topology: standalone-battery (path C), boost LED driver (TI LM3410 or TPS61169) drives 5 LEDs in series at 300 mA CC from 1S LiPo, ATtiny412 MCU drives DIM as a strict-on/off digital line at the 100 Hz chip rate (no PWM dim control), 2-bit code-select via solder jumpers or DIP switch. Boost driver EN is held HIGH by the supervisor (boost stays warm); MCU never touches EN. Battery insertion/removal is the power switch (no mechanical on/off)** | operator direction 2026-05-14 / refinement 2026-05-17 ("the part should be warm with strictly on/off from code — when battery is in, blink. That's it.") | M2 effort is transitory; standalone design eliminates all airframe wiring + EMI envelope + dual-mode safety complexity. Battery-as-switch eliminates the on/off mechanism. Single-mode at 300 mA is IEC 62471 RG0 at typical viewing distances. No PWM dimming hardware required at single-mode 300 mA — MCU output is a single GPIO carrying the LUT bit at chip rate |
