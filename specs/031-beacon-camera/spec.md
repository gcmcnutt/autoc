...
internal JST-PH 2.0 mm socket → 1S rail (V_BAT)
   │
   │   ┌── voltage supervisor IC monitors V_BAT (3.3 V UVLO per FR-1.7)
   │   │   open-drain output: HIGH-Z (released) when V_BAT > 3.3 V; LOW (pull-down) below
   │   │
   ...
   │      │ ─── Gold-code LUT bit at 100 Hz chip rate, GPIO in open-drain mode:
   │      │       chip = 1 → release (high-Z); pull-up holds DIM HIGH
   │      │       chip = 0 → drive LOW
   │      ▼
   │
   └─────────── DIM line (wired-AND, all drivers open-drain) ───────────┐
        V_BAT ──── 10 kΩ pull-up ────────────────────────────────────────┤
        MCU GPIO (open-drain, LSB of code LUT) ──────────────────────────┤
        supervisor open-drain output (LOW at UVLO; HIGH-Z otherwise) ────┤
                                                                          ▼
                                                              LM3410X DIM pin
       
      Truth table on DIM:
        supervisor=HIGH-Z (V_BAT OK) & MCU=HIGH-Z (chip=1) → DIM=HIGH → LEDs ON
        supervisor=HIGH-Z            & MCU=LOW   (chip=0) → DIM=LOW  → LEDs OFF
        supervisor=LOW   (UVLO)      & MCU=any            → DIM=LOW  → LEDs OFF
       
      (All three drivers on DIM are open-drain (or high-Z); the pull-up resistor
       is the only DC path from V_BAT to DIM. Any driver pulling LOW wins;
       all releasing = DIM goes HIGH via pull-up. No DC contention path exists.
       Soft-start at every chip-1 transition takes ~20 µs out of 10 ms = 0.2 % —
       negligible waveform impact.)
      (battery removal = V_BAT collapses = pull-up loses supply = DIM = 0 = LEDs off)
```
...
| MCU | **Microchip ATtiny412** | SOIC-8 | 5 × 4 mm | 1.5 mm | 8 pins: V+, GND, **code-out GPIO (single digital line to LM3410X DIM — open-drain emulated, LUT bit at 100 Hz chip rate; no PWM peripheral required)**, 2× code-select inputs, UPDI, reset, 1 spare. MCU does NOT participate in the UVLO cutoff path — supervisor's open-drain output gates DIM independently in the wired-AND topology (FR-1.7 #4). **Runs directly off V_BAT** (1.8-5.5 V operating range covers the 3.0-4.2 V LiPo swing); no LDO needed. Internal 20 MHz RC oscillator — see NFR-4 clock-drift requirements |
| Voltage supervisor (LiPo UVLO, **safety-critical**) | **Microchip MCP1316T-29LE/OT** or **TI TPS3839K33** or **Diodes APX803-31SAG** | SOT23-5 or SOT-353 | 2.9 × 2.8 mm or 2.0 × 2.0 mm | 1.1 mm | Trips at 3.3 V with ~100 mV hysteresis (matches FR-1.7 #4 threshold); **open-drain output wires onto the LM3410X DIM pin** (with the 10 kΩ pull-up + MCU open-drain GPIO — wired-AND) and pulls DIM low at undervoltage. Quiescent current <5 µA. **Hardware-level UVLO independent of MCU** — protects the 1S LiPo from over-discharge damage |
| DIM-line pull-up | **10 kΩ ±5 % 0603** to V_BAT | 0603 | 1.6 × 0.8 mm | 0.5 mm | Holds DIM HIGH when MCU GPIO + supervisor are both high-Z (wired-AND idle). Pull-up RC ≈ 10 kΩ × ~10 pF parasitic ≈ 100 ns rise on chip=1, negligible vs the 10 ms chip period. 100 kΩ alternative for slightly lower V_BAT idle draw — only matters if every 100 µA counts for runtime |
...
5. **MCU + supervisor decoupling**: 1 µF + 100 nF at the ATtiny412 VCC pin, 100 nF at the voltage-supervisor VCC pin (per FR-1.2.1 BOM). Filters boost-switching transients on the shared V_BAT rail — important because the MCU and supervisor run directly off V_BAT with no LDO to provide PSRR isolation.
...
4. **Battery low-voltage cutoff (LiPo protection — safety-critical)**: the pod SHALL stop driving the LEDs when the 1S cell drops to **3.3 V** (firm UVLO threshold, see Decisions Locked). This is **mandatory hardware-level UVLO** — must trip independently of the MCU (so a crashed/hung MCU does not strand the boost driver still draining the cell into damage range). Implementation contract:
   ...
   - **Voltage supervisor IC** (e.g., **Microchip MCP1316T-29LE/OT**, **TI TPS3839K33**, **Diodes APX803-31SAG**) monitors the 1S rail. **Open-drain output is required** — the supervisor pulls the LM3410X **DIM** pin low at the UVLO trip point, overriding the MCU's chip-rate GPIO. (The LM3410 has no separate EN pin; DIM serves both shutdown and dimming functions — see FR-1.2.1 BOM driver-IC row.) All three drivers on DIM are open-drain or high-Z — MCU GPIO configured as open-drain (chip=1 releases, chip=0 drives LOW), supervisor open-drain output, and a 10 kΩ pull-up to V_BAT. Wired-AND truth table: supervisor HIGH-Z (V_BAT > 3.3 V) AND MCU HIGH-Z (chip=1) → DIM HIGH; MCU drives LOW OR supervisor drives LOW → DIM LOW. **No DC contention path exists.** Under UVLO, the LM3410X enters its ~80 nA shutdown state (datasheet I_Q-shutdown), giving very low cell drain post-trip.
   ...
   - **NOT firmware-controlled**: the MCU does NOT participate in the cutoff path. Even if the MCU crashes, hangs, or holds its DIM-driving GPIO stuck HIGH, the boost driver SHALL still shut off at the supervisor threshold — the supervisor's open-drain output pulls DIM LOW with low impedance, overriding the MCU's push-pull HIGH via the wired-AND topology (supervisor wins). The MCU MAY monitor a separate ADC reading of the 1S rail for diagnostic blink-codes (FR-1.2.1 diagnostic LED) but the supervisor is the load-bearing safety element.
   - Boost driver candidates' built-in V_IN UVLO is **insufficient**: LM3410 trips at ~2.3 V rising / ~1.7-1.9 V falling (datasheet §6.5), TPS61169 ~2.5 V, AL8807 ~6 V (designed for higher-input applications) — none match the 3.3 V LiPo-safe threshold. External supervisor is required regardless of driver choice.
...
| **LED drive topology: standalone-battery (path C), TI LM3410**X** (1.6 MHz, SOT-23-5) boost LED driver drives 5 LEDs in series at 300 mA CC from 1S LiPo. ATtiny412 MCU drives the LM3410X **DIM pin** (the only shutdown control — LM3410 has no separate EN) as a strict-on/off digital line at the 100 Hz chip rate (no PWM dim control), via an open-drain GPIO. Voltage supervisor open-drain output wires in parallel onto DIM with a 10 kΩ pull-up to V_BAT (wired-AND topology): supervisor pulls DIM LOW at the 3.3 V UVLO threshold, overriding the MCU. 2-bit code-select via solder jumpers or DIP switch. Battery insertion/removal is the power switch (no mechanical on/off).** | operator direction 2026-05-14 / refinement 2026-05-17 ("the part should be warm with strictly on/off from code — when battery is in, blink. That's it.") / **architecture corrected 2026-05-18 per LM3410 datasheet (T007a)**: LM3410**X** has no separate EN pin, so the "warm" intent is satisfied via the IC's 20 µs soft-start (0.2 % of a 10 ms chip — negligible distortion) rather than a held-EN architecture; DIM-wired-AND with supervisor open-drain dominance gives the same behavioral guarantee. | M2 effort is transitory; standalone design eliminates all airframe wiring + EMI envelope + dual-mode safety complexity. Battery-as-switch eliminates the on/off mechanism. Single-mode at 300 mA is IEC 62471 RG0 at typical viewing distances. No PWM dimming hardware required at single-mode 300 mA — MCU output is a single open-drain GPIO carrying the LUT bit at chip rate. Under UVLO, supervisor pulls DIM LOW → LM3410X enters ~80 nA shutdown. |
...
| **LiPo UVLO: 3.3 V hardware-level cutoff via voltage supervisor IC gating the LM3410X DIM pin (wired-AND with MCU GPIO + 10 kΩ pull-up), ~100 mV hysteresis, MCU-independent** | safety clarification 2026-05-14 / architecture corrected 2026-05-18 per LM3410 datasheet (T007a) | LiPo over-discharge (below 3.0 V) damages the cell and risks fire on next charge. Hardware supervisor (not firmware) ensures cutoff even if MCU crashes. 3.3 V is storage-safe — operator can leave spent battery overnight without immediate recharge. Built-in V_IN UVLO of common boost drivers (LM3410 at ~2.3 V per datasheet §6.5) is too low — external supervisor is mandatory regardless of driver choice. Supervisor gates DIM rather than EN because LM3410 has no separate EN pin (DIM is the only shutdown control); the wired-AND topology gives supervisor's pull-down dominance over MCU's GPIO state |
