...
- **Broader context**: this investigation was a sidetrack from the M2-reproducibility line of work. The next moves on the main path:
  ...
  2. **M2 sim playback parity with M1**: the load-bearing experiment after the basin work — run M2 (tracker mode) playback over the same scenario variations that M1 trained on (same wind seeds, entry poses, rabbit-speed draws, joint-PRNG samples). If M2 results track M1's training-time signal closely, the whole M2 architecture is validated; if they diverge, we know perception or some other M2-side delta is leaking. This is the real question; the basin-landscape work is housekeeping that lets us cleanly attribute M2 divergence to *code* rather than *seed luck*.
