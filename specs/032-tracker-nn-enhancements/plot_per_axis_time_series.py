#!/usr/bin/env python3
"""030 per-axis aggressiveness time-series — pt/rl/th evolution across gens.

Adapted from 029's plot_per_axis_time_series.py. Body unchanged — column
lookup is by header name (outPt/outRl/outTh + qw..qz + Scn +
Pth/Wnd:Step:), all present in tracker-mode data.dat. Only defaults
differ (output path, label).
...
def main():
    ...
    ap.add_argument("--out", type=Path,
                    default=Path("specs/032-tracker-nn-enhancements/per_axis_time_series.png"))
    args = ap.parse_args()
