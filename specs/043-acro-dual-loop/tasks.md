...
- [ ] **flash the xiao** — ⚠️ no FC flash this cycle, so the GPS can stay connected
- [ ] **T069-t3** Bench-verify ⭐ flash identity must read `weight_id=a5d097da18bf33ed` (t2 was `3af8e3ab787b75a5`); re-assert rate parity after flashing; ACRO purity; arm-C `axisF[1]` saturation. ⚠️ **Watch-item**: t2 railed nose-down at 93% through the whole bench engagement (OOD saturation from static airspeed ≈0.2 vs 13 m/s trained). t3 took the pitch rail 33.5% → **0.0%** on the *cold-start* OOD, so it may look better — ⛔ but that is a **different** OOD axis, so a clean bench does not confirm the static-airspeed hole is closed.
- [ ] **T070** Fly — proceed to the existing T070–T077 chain below.
