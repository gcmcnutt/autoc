...
- **FR-008b**: Candidate lexicase axes for this multi-objective framework are **energy** (primary, this feature), **stability** (FR-008), and **M2 hull-crash-cost**. The hull-crash-cost is the stranded "033 phase-2" kamikaze penalty: in tracker (M2) bakes, hull-strikes grow *monotonically with tracking skill* (030: ~1→11/gen; 032: ~3× faster) — the chase craft learns to fly *into* the target as it sharpens, which **gates real-flight deployment**. It is a *fitness dimension* (belongs here, NOT in 036/island-selection work), and like energy/stability it MUST be a lexicase axis, not a scalar penalty (scalar composition collapses to a Pareto corner). Candidate metric: `scenario_score × 0.5` on hull strike, or a dedicated `crash_cost` field on `ScenarioScore`. The M2 run quantifies the current hull-escalation rate (`#GenCrash hullStrike=N`) — record whether to fold crash-cost into 035 now or as an immediate 035 follow-on.
