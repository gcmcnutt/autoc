...
- **Baseline**: the tracking-only run(s) that energy bakes are compared against — the M1 baseline (034-delivered, pop=8000/wind=36 + craft variations) for FR-005, and the corresponding M2 tracking-only tracker baseline for FR-005b. Same objective in both modes (on-point tracking + minimum energy).
