...
- [ ] T036 [US1] M1 energy bake (`autoc.ini`, pop=8000/wind=36, `LexicaseEpsilonMode=mad`, energy axis on); budget 2–3 bakes for the basin lottery (watch throttle-σ=0.000 stuck tell); compare vs the M1 tracking-only baseline using the **per-axis aggressiveness distributions** (`plot_per_axis_time_series.py` → `per_axis_aggressiveness` PNGs) — variation/config-stable comparator that isolates energy from the MAD-ε change; expect movement toward goal on **all 3 controls (pitch/roll/throttle)** (FR-005).
