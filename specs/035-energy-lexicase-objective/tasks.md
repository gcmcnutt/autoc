...
- [X] T011 Add `.zst` inflate to the dmp fetch paths in `tools/nnextractor.cc` and `tools/renderer.cc` (via the shared selector + decompress). _(DONE: nnextractor + source_dmp_loader route through `s3GetDmpBlob`; renderer fetch fixed in 35ad056 — `updateGenerationDisplay` now calls `s3GetDmpBlob` (was hardcoding `.dmp` + no inflate → 404'd on `.dmp.zst`).)_
...
- [X] T022 Flip `S3Bucket` per ini to `autoc-m1` / `autoc-m2` / `autoc-eval` (`autoc.ini`, `autoc-basic-m1.ini`, `autoc-tracker.ini`, `autoc-eval.ini`, `autoc-eval-tracker.ini`, `autoc-eval-visual.ini`) per `contracts/ini-config.md` — config-only, after T021.
...
- [X] T036 [US1] M1 energy bake (`autoc.ini` as-is = pop=5000/wind=49, the 034-validated config that tracks pastonly3 — NOT 8000/36, which throttle-pegs; `LexicaseEpsilonMode=mad`, energy axis on); budget 2–3 bakes for the basin lottery (watch throttle-σ=0.000 stuck tell); compare vs the M1 tracking-only baseline using the **per-axis aggressiveness distributions** (`plot_per_axis_time_series.py` → `per_axis_aggressiveness` PNGs) — variation/config-stable comparator that isolates energy from the MAD-ε change; expect movement toward goal on **all 3 controls (pitch/roll/throttle)** (FR-005). _(DONE 2026-06-08: t6 = `autoc-…2026-06-06T19:45:15.319Z`, gen 800, best −37690, 0% crash, 294/294. **Energy-works** — throttle off Vmax-pinning (mag 0.93→0.72), pitch de-saturated, lexicase specializes vs same-config 034 scalar (pitch/throttle dctrl ↓, roll concentrated). 1 clean bake sufficed (no lottery stall at this config). Run pinned `retain=keep` (800 objs). See outcome.md.)_
- [X] T037 [US1] M2 energy bake (`autoc-tracker.ini`, same objective: on-point + min energy); budget 2–3 bakes; compare vs the **pinned M2 tracking-only baseline = 032-phase1** (`autoc-9223370257807536859-2026-05-17…`) or a fresher confirmed non-stuck climber (M2 is lottery-prone — stalls / dead neurons — so the baseline MUST be a confirmed climber); use the same per-axis comparator; capture `#GenCrash hullStrike=N` escalation (seeds the future hull-crash feature) (FR-005b). _(DONE — QUALITATIVE 2026-06-08: t7 = `autoc-…2026-06-08T15:44:25.312Z`, stopped gen 207, best −14760, 268/294, avgMaxStreak 9.0, hullStrike 12 (6→12 escalation over run — seeds hull-crash feature). **Energy-works** — no tracking collapse; same per-axis allocation as M1 (throttle ultra-smooth dctrl 0.088 though pinned-high mag 0.94, change-rate on cheap roll). Ran on **off-nominal scenario set** (short-source skip bug `c95887e` → 294-slot modulo reshuffle), so tracking-quality numbers are qualitative not milestone; clean converged M2 rerun deferred to the skip-fix = **037 prework**. See outcome.md M2 section.)_
- [X] T038 [US1] Write the outcome doc `specs/035-energy-lexicase-objective/outcome.md`: classify EACH mode as energy-works / tracking-collapses / energy-unmoved with per-scenario evidence AND the per-axis aggressiveness distribution shift on all 3 controls (SC-001/002, FR-006); record the total-energy (PE/KE) go/no-go (FR-007); pin milestone runs `retain=keep` and record their S3 prefixes (Principle VIII). _(DONE 2026-06-08: M1 + M2 sections written + combined verdict (both modes = energy-works); total-energy GO recorded; t6 M1 pinned + prefix recorded; M2 clean-bake pinning deferred to skip-fix per caveats.)_
