...
| **CAD — KiCad schematics** (031 receiver/eval) | KiCad 10 (eeschema GUI for placement/ERC; `kicad-cli sch export pdf` for artifacts) | ⚠️ **NOT installed on this Windows host** (verified 2026-07-22) — the 2026-06 captures ran on the other system. Repo `.kicad_sch` files are hand-editable s-expressions; generation/edits can be done WSL-side in Python (see `cad/beacon-receiver/beacon-receiver-daylight.kicad_sch` provenance), with GUI placement + ERC wherever KiCad lives. | open in GUI on the KiCad host; document the path here when installed locally |
