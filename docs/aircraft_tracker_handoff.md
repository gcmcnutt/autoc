...
>
> **RESOLVED 2026-06-22 (037 T047 write-back): the control rate is 20 Hz** (GO verdict + 0.8 s history window; see [`../specs/037-20hz-control-loop/outcome.md`](../specs/037-20hz-control-loop/outcome.md)). So the acquisition budget = **≤25 ms** (½ the 50 ms control tick). Co-design chip rate / code length against **480 fps + this 20 Hz / ≤25 ms budget** (50 Hz remains a documented stretch, not the target). The current 031 1-bit phase already uses 20 Hz in its budget math ([`../specs/031-beacon-camera/acquisition-research-plan.md`](../specs/031-beacon-camera/acquisition-research-plan.md) §8), and the per-beacon-independent decode (R7) is carried in that plan's §5.
>
