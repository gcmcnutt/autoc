#!/usr/bin/env python3
"""#GenDiag multi-panel chart — operator-instrumentation per-gen diagnostics.

Carried forward from `specs/032-tracker-nn-enhancements/plot_gen_diag.py`. Parses
the `#GenDiag` lines emitted once per generation with 18 metrics:

  loss_total, far, angle, over, hull,
  avgVis, avgInRamp,
...
034 fixup: as of 034 T052 the gen markers (`#NNGen`, `#GenDiag`, …) live in the
run **.log**, where each line carries a `<timestamp>: <info> ` logger prefix.
The 032 parser gated on `line.startswith("#GenDiag ")`, which the prefix defeats,
so this version matches `"#GenDiag "` anywhere in the line. The field set is
unchanged from 032 (the `fov`/`cone` taxonomy branches were already dead code),
so committed `.stc` snapshots that still carry bare `#GenDiag` tokens parse
identically and remain valid `--in` inputs.

Usage:
    python3 plot_gen_diag.py [--in LOG] [--total-gens N] [--label NAME]
                              [--out PNG] [--variation-ramp N]
"""
...
def parse_gen_diag(path: Path):
    ...
    for line in path.read_text().splitlines():
        # 034: log lines are prefixed with `<timestamp>: <info> `, so match the
        # token anywhere rather than at the start. The `<timestamp>` carries no
        # `key=value` pairs, so FIELD_RE over the whole line stays clean.
        if "#GenDiag " not in line:
            continue
...
def main():
    ...
    ap.add_argument("--in", dest="inp", type=Path,
                    default=Path("logs/autoc-034-m2-tracker-origm1-r1.log"),
                    help="run .log (or historical .stc) carrying #GenDiag lines")
    ap.add_argument("--label", default="034-m2-tracker-origm1-r1")
    ap.add_argument("--total-gens", type=int, default=800)
    ap.add_argument("--out", type=Path, default=None,
                    help="default: specs/034-energy-objective-cleanup/<label>_gen_diag.png")
    ...
    if args.out is None:  # derive from --label (lexicographic 034-t<N>-<reason> convention)
        args.out = Path(f"specs/034-energy-objective-cleanup/{args.label}_gen_diag.png")
