...
def main(stdscr):
    ...
    while True:
        ...
        for (tx, ty, tc) in trail[:-1]:
            ...
            if 1 <= gx <= GW and 3 <= gy < 3+GH: stdscr.addch(gy, gx, '*', COL.get(tc, 0))
        if last:
            x, y = last["x"], last["y"]; code = last["code"]
            gx = 1 + int(x / PXW * GW); gy = 3 + int(y / PXH * GH)
            ch = '@' if last.get("lock") else '?'
            if 1 <= gx <= GW and 3 <= gy < 3+GH: stdscr.addch(gy, gx, ch, curses.A_BOLD | curses.A_REVERSE | COL.get(code, 0))
            ...
            for k, s in enumerate(info): stdscr.addstr(3+GH+3+k, 0, s[:curses.COLS-1], COL.get(code, 0) if k == 0 else 0)
            stdscr.addstr(3+GH+8, 0, " convention: code A = PORT (red)   code B = STARBOARD (green)   -- aviation nav lights; renderer matches"[:curses.COLS-1])
        else:
