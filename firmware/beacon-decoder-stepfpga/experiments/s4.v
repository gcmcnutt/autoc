...
// s4 = s3 + REAL external emitter in the loop: an ATtiny416 drives its Gold code onto P8 (via a 5V->3.3V divider);
// s4 muxes it into the code-B path BEFORE the analog model, so the code-B correlator + DPLL lock on the real,
// INDEPENDENT RC clock -> rateB telemetry / LEDr = the live emitter oscillator measurement (E4 / A2-osc).
// DIP4 (was synthetic-B skew, meaningless for a real emitter) now selects EXTERNAL(open/default) vs SYNTHETIC B.
module s4_top (input clk12,
               input sw1, input dip2, input dip3, input dip4,   // DIP1(M7) DIP2(M8) DIP3(M9) DIP4(M10=ext/synth B)
               input k1, input k2, input k3, input k4,
               input ext_code, output sync_pin,                 // P8 = real emitter code IN (was code_pin OUT); N8 epoch
  ...
  // (P8 is now the external-emitter INPUT — see codeB mux below; no synthetic code output in s4)
  ...
  reg [2:0] exts=0;                                            // sync the async external emitter code into clk12
  always @(posedge clk12) exts <= {exts[1:0], ext_code};
  reg [1:0] cAs=0,cBs=0,blk_s=0; always @(posedge clk12) begin cAs<={cAs[0],codeA}; cBs<={cBs[0],codeB}; blk_s<={blk_s[0],blanked}; end
  // code B feeding the analog model = the REAL external emitter (DIP4 open/default) OR the synthetic emitter B
  wire codeA_rx=cAs[1], codeB_rx = extsel ? exts[2] : cBs[1], blank_rx=blk_s[1];
