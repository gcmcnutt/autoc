...
module bcn_tx (
  ...
  input  wire [15:0] recA,  input wire [15:0] recB,    // recovery latency (samples to last lock)
  output wire txd);
  // Frame: BCN,seq4,adc4,corrA6,lockA1,marginA1,corrB6,lockB1,marginB1,rateA5,rateB5,recA5,recB5\n
  ...
  integer j;
  initial begin
    for (j=0;j<64;j=j+1) lbuf[j]=8'h20;
    lbuf[0]="B"; lbuf[1]="C"; lbuf[2]="N"; lbuf[3]=",";
    lbuf[8]=","; lbuf[13]=","; lbuf[20]=","; lbuf[22]=","; lbuf[24]=","; lbuf[31]=","; lbuf[33]=",";
    lbuf[35]=","; lbuf[41]=","; lbuf[47]=","; lbuf[53]=","; lbuf[59]=8'h0A;   // ,rateA,rateB,recA,recB,newline
  end
  localparam integer LEN = 60;
    ...
    case (state)
      ...
      2'd1: begin                                       // load field fi
        ...
        case (fi)
          ...
          4'd9:  begin wv <= {4'b0, rateB}; w <= 3'd5; base <= 6'd42; end
          4'd10: begin wv <= {4'b0, recA};  w <= 3'd5; base <= 6'd48; end
          default: begin wv <= {4'b0, recB}; w <= 3'd5; base <= 6'd54; end
        endcase
        state <= 2'd2;
      end
      2'd2: begin                                       // MSD-first power-of-10 subtraction
        if (wv >= curpow[19:0]) begin wv <= wv - curpow[19:0]; digit <= digit + 1'b1; end
          ...
          if (dp == w-3'd1) begin
            if (fi == 4'd11) begin si <= 0; state <= 2'd3; end
