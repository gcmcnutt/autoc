// S7 -- TWO-STREAM correlator, code A now DIGITALLY INJECTED. Code A is a fully-synthetic reference on its own
...
`endif
  ...
  // s7 A4d-8: GEAR-SHIFTED DC tracker — alpha 1/256 while LOCKED (tau 533 ms, signal-protecting), 1/32 while
  // UNLOCKED (tau 67 ms) so pedestal/amplitude steps re-center ~8x faster (s6 measured: 10x down-step buried
  // the signal under DC bias ~1 s, full settle 2.5-3 s; projected ~0.5 s). Fast cutoff ~2.4 Hz vs the 6.5 Hz
  // word rate (2.7x apart) and only during acquisition. No overflow: acc' = 248*dc + 8*s stays in [0,256*4095].
  wire signed [12:0] ddA = $signed({1'b0, injA})    - $signed({1'b0, dcA});
  wire signed [12:0] ddB = $signed({1'b0, sampB_g}) - $signed({1'b0, dcB});
  always @(posedge clk12) begin
    rdy <= 1'b0;
    if (valid) begin
      ...
      dcA_acc <= dcA_acc + (lockedA ? {{7{ddA[12]}}, ddA} : {{4{ddA[12]}}, ddA, 3'b000});
      dcB_acc <= dcB_acc + (lockedB ? {{7{ddB[12]}}, ddB} : {{4{ddB[12]}}, ddB, 3'b000});
  ...
  // s7 A4d-7: MIN-ENERGY LOCK GATE. On a truly-silent input the energy denominator sits near its |1 floor and
  // q = 10*best/energy can spuriously spike >= GOOD -> false lock=2 flashes during long dark (seen 2/3 of the
  // 8 s rungs, hardwire era). Gate "good" on per-period energy >= ENERGY_MIN: dark floor ~1-20, weakest REAL
  // lock observed ~300 (P=8 bounce) -> 64 splits them with >4x margin both ways.
  localparam [21:0] ENERGY_MIN = 22'd64;
  wire eokA = (besteA >= ENERGY_MIN), eokB = (besteB >= ENERGY_MIN);
